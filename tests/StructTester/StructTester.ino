 ...
 * you get out of the Design Tool
 */
#include <Audio.h>
...
void setup() 
{
  ...
  // make a square of rotating ellipses
  setQuad(qoscs[0], 120.0f, WAVEFORM_SINE);
  setGain(qoscs[0], 0.2f);
  setRatio(qoscs[0],1.005f);
  setQuad(qoscs[1],  30.0f, WAVEFORM_SQUARE);
  setGain(qoscs[1], 0.7f);

  // make them wibble
  OscMod.Mod1.begin(1.0f, 0.5f, WAVEFORM_SINE);
  setModLevels(qoscs[0].X, 0.75f,  0.25f, 0.0f);
  setModLevels(qoscs[0].Y, 0.75f, -0.25f, 0.0f);
  OscMod.Mod2.begin(1.0f, 1.6f, WAVEFORM_TRIANGLE);
  setModLevels(qoscs[1].X, 0.7f, 0.0f, -0.2f);
  setModLevels(qoscs[1].Y, 0.7f, 0.0f,  0.2f);
